...
- The entrypoint drops permanently to the non-root `sandbox` user before the
...
├── container/
│   ├── Containerfile
│   ├── entrypoint.py       # privilege drop, optional offline mode, server exec
│   ├── requirements.txt    # image dependencies
│   └── server.py           # authenticated standard-library HTTP server
└── tests/                  # direct API, subprocess, and lifecycle checks
```
...
```python
from tapestry.workspace.sandbox_manager import sandbox_session

config = {
    "image": "python-sandbox:latest", "port": 8080, "workspace": "/workspace",
    "memory_limit": "2g", "cpu_limit": 1.0, "egress_policy": "direct",
}
with sandbox_session(config) as endpoint:
    print(endpoint.get("base_url", ""))
    # Supply endpoint.get("auth_token", "") as Authorization: Bearer <token>.
```
