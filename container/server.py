...
def workspace_tree():
    ...
    result = subprocess.run(["find", ".", "-maxdepth", "3", "-type", "f"], capture_output=True, text=True, cwd=WORKSPACE)
...
def git_log():
    """Recent git history."""
    result = subprocess.run(["git", "log", "--oneline", "-20"], capture_output=True, text=True, cwd=WORKSPACE)
    return result.stdout
